                    ...
                    foreach ($selecionados as $item) {
                        ...
                        $sql = "-- Parametros de Entrada
                                        ...
                                        FORMAT(VALORFINAL, 'C', 'pt-br') AS VALORFINAL,
                                         ...
                                         (VALORFINAL - VALORIPI) + (CASE WHEN (CUSTOSERIAL < VALORFINAL) 
                                                    THEN ((VALOR - CUSTOSERIAL)*(IPI/100)) 
                                                    ELSE 0 END) VALORTOTAL_NUM,
                                        FORMAT((VALORFINAL - VALORIPI)  + (CASE WHEN (CUSTOSERIAL < VALORFINAL) 
                                                    THEN ((VALOR - CUSTOSERIAL)*(IPI/100)) 
                                                    ELSE 0 END), 'C', 'pt-br') VALORTOTAL,
                        ...
                        <tbody>
                            ...
                            while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                                ...
                                /* Calcular IPI */
                                $IPI =  ($row['VALORFINALNUM'] - $row['CUSTOSERIAL']) * 0.0975;
                                $ValorFinal = $row['VALOR'] + $IPI;
                                ...
                                $tabela .= $dbColor;
                                $tabela .= $dbTotal;
                                $tabela .= "<td class='currency'>" .$row['VALORTOTAL']. "</td>";
                                $tabela .= "</tr>";
